...
export const getServerSideProps = async() => {
    ...
    const res = await fetch(`https://newsapi.org/v2/${request.china.url}`)
...
function China( {data}: InferGetServerSidePropsType<typeof getServerSideProps>){
    if(!data){
        return(
                    ...
                    <Content className="site-layout" style={{ padding: '0 50px', marginTop: 64, height: "100vh" }}>
                        <Row>
                            ...
                            <Col xxl={14} xl={14} lg={18} xs={20} md={18} sm={18} style={{ textAlign: "center" }}>
                                <div>
                                    <Title level={3} data-aos="zoom-out" data-aos-delay="200" data-aos-once style={{paddingTop:"50px"}}>HEADLINES/CHINA</Title>
                                </div>
                            </Col>
                            <Col xxl={5} xl={5} lg={3} xs={2} md={3} sm={3} />
                        </Row>
